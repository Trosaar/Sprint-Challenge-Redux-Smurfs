...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  ...
  Actions are all the functions that will influence state. The reducer takes that action and interprets it or reduces it down to something that state and therefor the rest of the application can understand. The store is simply a global version of state where we can connect it to individual components instead of passing it around with props. The store is known as a single source of truth because it allows you to accumulate the state from everything and house it in one spot.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

  Application state is the whole concept of redux or the store where you have one state to be shared for the whole application. Component state is simply when you house state within a single component to be used locally instead of globally. A good time to use component state is when your state is only needed for that specific component. We've seen it most when implementing forms.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

  redux-think is a middleware that allows us to create asynchronous action-creators by giving us access to the dispatch function. This gives us more options for makings server requests.
